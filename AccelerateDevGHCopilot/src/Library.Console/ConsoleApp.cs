...
public class ConsoleApp
{
    ...
    async Task SearchBookAvailability()
    {
        ...
        var book = _jsonData.Books?.FirstOrDefault(b => b.Title != null && b.Title.Equals(titleInput, StringComparison.OrdinalIgnoreCase));
        ...
        // Get BookItem for this book
        var bookItem = _jsonData.BookItems?.FirstOrDefault(bi => bi.BookId == book.Id);

        if (bookItem == null)
        {
            Console.WriteLine($"{book.Title} has no copies in the library system.");
            return;
        }
